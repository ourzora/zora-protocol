...
import {ZoraCreatorRedemptionMinterStrategy} from "./ZoraCreatorRedemptionMinterStrategy.sol";
import {Ownable2StepUpgradeable} from "../../utils/ownable/Ownable2StepUpgradeable.sol";
import {IZoraCreator1155} from "../../interfaces/IZoraCreator1155.sol";
import {SharedBaseConstants} from "../../shared/SharedBaseConstants.sol";

contract ZoraCreatorRedemptionMinterFactoryImpl is UUPSUpgradeable, Ownable2StepUpgradeable, SharedBaseConstants, IVersionedContract {
    ...
    function createMinter(address _creatorContract, bytes32 _salt) external returns (address) {
        ...
        ZoraCreatorRedemptionMinterStrategy minter = new ZoraCreatorRedemptionMinterStrategy{salt: _salt}(_creatorContract);
    ...
    function predictMinterAddress(address _creatorContract, bytes32 _salt) external view returns (address) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                address(this),
                _salt,
                keccak256(abi.encodePacked(type(ZoraCreatorRedemptionMinterStrategy).creationCode, abi.encode(_creatorContract)))
            )
        );
