...
library ZoraCreator1155Attribution {
    ...
    function recoverSigner(PremintConfig calldata premintConfig, bytes calldata signature, address erc1155Contract) public view returns (address signatory) {
        bytes32 encoded = hashPremintConfig(premintConfig);
        // first validate the signature - the creator must match the signer of the message
        return recoverSignerHashed(encoded, signature, erc1155Contract);
    }

    function recoverSignerHashed(bytes32 hashedPremintConfig, bytes calldata signature, address erc1155Contract) public view returns (address signatory) {
        // first validate the signature - the creator must match the signer of the message
    ...
    function premintHashData(bytes32 encoded, address verifyingContract, uint256 chainId) public pure returns (bytes32) {
    ...
    function hashPremintConfig(PremintConfig calldata premintConfig) public pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
