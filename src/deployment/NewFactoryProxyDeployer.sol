...
contract NewFactoryProxyDeployer is EIP712 {
    ...
    bytes32 public constant DEPLOYMENT_SALT = bytes32(0x0000000000000000000000000000000000000000668d7f9eb18e35000dbaaa0f);
    ...
    bytes32 constant DOMAIN = keccak256("createFactoryProxy(bytes32 proxyShimSalt,bytes32 factoryProxySalt,bytes32 factoryImplAddress,address newOwner)");
    ...
    function recoverSignature(
        ...
        bytes32 digest = hashedDigest(proxyShimSalt, factoryProxySalt, factoryImplAddress, newOwner);
    ...
    function createFactoryProxyDeterminstic(
        ...
        address signer = recoverSignature(proxyShimSalt, factoryProxySalt, factoryImplAddress, newOwner, signature);

        requireContainsCaller(signer, proxyShimSalt);

        return _createAndInitializeNewFactoryProxyDeterminstic(proxyShimSalt, factoryProxySalt, expectedFactoryProxyAddress, factoryImplAddress, newOwner);
    }
}
