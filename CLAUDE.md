...
For comprehensive technical architecture and monorepo details, see MONOREPO_ARCHITECTURE.md.
...
- Prioritize user goals: What does someone want to accomplish? (for example, "Create a coin", "Understand rewards")
...
- Use action-oriented headings (for example, "Creating a Coin" instead of "Coin Creation")
- Lead with benefits and outcomes, not implementation details
- Include decision-making guidance (for example, "Use this configuration if...", "Choose based on...")
- Link strategically to related concepts and next steps

Documentation Contract Lint

Contract documentation guidelines should be enforced in CI, not treated as review-only guidance.

Add a lightweight documentation lint check that runs when contract-related documentation changes. This lint should help catch common documentation issues early and reduce reviewer burden.

Suggested trigger paths include:
...
The lint should validate common issues such as:

- Reference-oriented headings that should be action-oriented
- Missing practical examples in user-facing contract documentation
- Mentions or recommendations of deprecated methods
- Redundant content that should instead link to architecture or related docs
- Missing links to related concepts or next steps
- Missing decision guidance where readers need help choosing between options

Suggested implementation:

- Add a script at .github/scripts/lint-contract-docs.sh
- Run it from a dedicated GitHub Actions workflow for documentation changes
- Emit warnings for style improvements and errors for prohibited content such as deprecated recommendations

Suggested script behavior:

- Detect changed markdown files in the PR diff
- Warn on headings like "Creation", "Configuration", "Methods", "Parameters", or "Overview" when they are used as static reference headings
- Error if deprecated methods are recommended
- Warn when user-facing contract documentation discusses contracts, rewards, or coins without any practical example blocks
- Optionally flag repeated sections that should link to MONOREPO_ARCHITECTURE.md or related pages instead of duplicating content

Suggested workflow behavior:

- Run on pull_request for matching documentation paths
- Check out the repository with sufficient history to diff against the base branch
- Execute the documentation lint script
- Surface warnings inline in GitHub Actions output

Optional enhancement:

Integrate this check with the existing Claude Code Review workflow so that contract documentation changes also receive semantic review for:
...
- Redundant content
- Missing related links or next steps
...
For technical details on build system architecture and package patterns, see MONOREPO_ARCHITECTURE.md#build-system-architecture.
...
For detailed build optimization strategies and dependency analysis, see MONOREPO_ARCHITECTURE.md#build-optimization-learnings.
...
- This generates a .changeset/*.md file that can be manually edited
...
For complete details on the protocol-deployments publication workflow, see MONOREPO_ARCHITECTURE.md#how-to-publish-a-contract-in-protocol-deployments.
...
- Contract name
- Version number (from the contract package, for example @zoralabs/limit-orders@0.2.5)
- Deployed address
...
Pre-deployment requirement: Before deploying contracts, ensure there are no outstanding changesets for the contract packages (for example @zoralabs/limit-orders, @zoralabs/coins). If there are, run pnpm update-version first to apply them. This ensures deployed contracts have the correct version numbers.

Deployment Instructions:

For detailed deployment instructions for specific contract types:

- Limit Orders: See packages/coins-deployments/LIMIT_ORDER_DEPLOYMENT.md for production deployments using Turnkey and dev deployments
...
- Navigate to package directory (for example, cd packages/coins)
...
- cd packages/{package-name} (for example, cd packages/coins)
...
- Each package may exclude different files (for example, *test*, *lib*, *uniswap*, etc.)
...
- script_name: The Forge script filename (for example, UpgradeCoinImpl.sol, DeployAllContracts.s.sol)
- chain: The chain name from viem/chains (for example, base, mainnet, sepolia, baseSepolia, zora, zoraSepolia)
...
- Source files: Individual package CHANGELOG.md files (for example, packages/coins/CHANGELOG.md)
- Generated files: Documentation changelog files (for example, docs/pages/changelogs/coins.mdx)
...
- Adds proper documentation titles (for example, "Coins Changelog")
...
- Avoid second person language: Never use "you", "your", "yours" in documentation. Use third person (for example, "the user", "developers", "coin creators") or imperative voice (for example, "Call the function", "Set the parameter") instead.
