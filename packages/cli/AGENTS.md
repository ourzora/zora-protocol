...
**API key is optional** — all commands work without one, but requests may be rate-limited. An API key also provides more accurate coin valuations in `zora balance` by using the SDK's liquidity-aware pricing.
