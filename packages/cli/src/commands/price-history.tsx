...
export const priceHistoryCommand = new Command("price-history")
  ...
  .action(async function (
    ...
    if (parsed.kind === "ambiguous-name") {
      ...
      if (ambResult.kind === "ambiguous") {
        if (json) {
          const [creatorPrices, trendPrices] = await Promise.all([
            fetchPriceHistory(ambResult.creator.address, interval as Interval),
            fetchPriceHistory(ambResult.trend.address, interval as Interval),
          ]);
          outputData(json, {
            json: {
              matches: [
          ...
          console.log("");
          await showPriceHistory(false, ambResult.trend, interval as Interval);
          console.log(
      ...
      const dataPoints = await showPriceHistory(
        json,
        ambResult.coin,
        interval as Interval,
      );
    ...
    const dataPoints = await showPriceHistory(json, coin, interval as Interval);
