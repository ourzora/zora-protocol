...
Prompts to create a new wallet or import an existing private key. Pass `--create` to generate one without prompting, or `--force` to overwrite an existing wallet.
