...
The `wallet.action` field reflects what happened: `created`, `imported`, `env_detected` (a `ZORA_PRIVATE_KEY` was already set), or `skipped` (an existing wallet was kept). The `path` field is present only for `created` and `imported`. The `apiKey` field is one of `saved`, `skipped`, `env_override`, or `already_set`.
