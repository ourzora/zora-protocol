...
| ------------ | -------------------------------------- |
| `identifier` | Coin address (`0x...`) or creator name |
...
The `--type` flag disambiguates when a name could match multiple coin types. When using an address, the type is resolved automatically.
...
```bash
npx @zoralabs/cli get jacob --type creator-coin
```
...
```bash
npx @zoralabs/cli get zora --type trend
```
...
```bash
npx @zoralabs/cli get jacob --type creator-coin --json
```
