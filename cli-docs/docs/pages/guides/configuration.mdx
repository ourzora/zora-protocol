...
The private key in `wallet.json` is stored in plain text — it is protected by file permissions (`0600`, owner read/write only), not encryption. Both files are versioned (`version: 1`) so the CLI can validate and migrate them.
