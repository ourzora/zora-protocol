...
export function CyclingHeadline({ term }: CyclingHeadlineProps) {
  ...
  if (snapshot.shown !== term) {
    setSnapshot({ shown: term, previous: reduceMotion ? null : snapshot.shown })
  }
  ...
  return (
    <h1 className={styles.headline}>
      ...
      <span aria-hidden="true" className={styles.mobileVisual}>
        <span>Your agent&rsquo;s</span>
        <span className={styles.mobileSlot}>
          <AnimatePresence initial={false}>
            <motion.span
              key={term}
              className={styles.mobileTerm}
      ...
      >
        ...
        <span className={styles.slot}>
          {snapshot.previous && (
            <span className={`${styles.word} ${styles.wordPrevious}`} aria-hidden="true">
          ...
          <motion.span
            ...
            initial={{
              clipPath:
                reduceMotion || snapshot.previous == null ? VISIBLE_CLIP : HIDDEN_CLIP,
            }}
            animate={{ clipPath: VISIBLE_CLIP }}
            transition={{
              duration: reduceMotion ? 0 : WIPE_DURATION,
              ease: EASE_OUT_QUINT,
            }}
          >
