...
contract ZoraCreator1155Test is Test {
    ...
    function test_FreeMintRewards(uint256 quantity) public {
        ...
        (uint256 totalReward, uint256 creatorReward, uint256 finderReward, uint256 originReward, uint256 zoraReward) = target.computeFreeMintRewards(quantity);

        address buyer = makeAddr("buyer");
        vm.deal(buyer, totalReward);

        vm.prank(buyer);
        target.mintWithRewards{value: totalReward}(simpleMinter, tokenId, quantity, abi.encode(recipient), address(0), address(0));
        ...
        assertEq(zoraRewards.balanceOf(zora), zoraReward + finderReward + originReward);
    }

    function test_FreeMintRewardsWithFinder(uint256 quantity) public {
        vm.assume(quantity > 0 && quantity < type(uint200).max);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalReward}(simpleMinter, tokenId, quantity, abi.encode(recipient), finder, address(0));
        ...
        assertEq(zoraRewards.balanceOf(zora), zoraReward + originReward);
    }

    function test_FreeMintRewardsWithLister(uint256 quantity) public {
        vm.assume(quantity > 0 && quantity < type(uint200).max);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalReward}(simpleMinter, tokenId, quantity, abi.encode(recipient), address(0), origin);
    ...
    function test_FreeMintRewardsWithFinderAndLister(uint256 quantity) public {
        vm.assume(quantity > 0 && quantity < type(uint200).max);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalReward}(simpleMinter, tokenId, quantity, abi.encode(recipient), finder, origin);
    ...
    function testRevert_InsufficientEthForFreeMintRewards(uint256 quantity) public {
        ...
        (uint256 totalReward, , , , ) = target.computeFreeMintRewards(quantity);
        ...
        target.mintWithRewards(simpleMinter, tokenId, quantity, abi.encode(recipient), address(0), address(0));
    }
    ...
    function test_PaidMintRewards(uint256 quantity, uint256 salePrice) public {
        ...
        (uint256 totalReward, uint256 finderReward, uint256 originReward, uint256 zoraReward) = target.computePaidMintRewards(quantity);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalValue}(fixedPriceMinter, tokenId, quantity, abi.encode(recipient), address(0), address(0));

        assertEq(address(target).balance, totalSale);
        assertEq(zoraRewards.balanceOf(zora), zoraReward + finderReward + originReward);
    }

    function test_PaidMintRewardsWithFinder(uint256 quantity, uint256 salePrice) public {
        vm.assume(quantity > 0 && quantity < 1_000_000);
        vm.assume(salePrice > 0 && salePrice < 10 ether);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalValue}(fixedPriceMinter, tokenId, quantity, abi.encode(recipient), finder, address(0));
    ...
    function test_PaidMintRewardsWithLister(uint256 quantity, uint256 salePrice) public {
        vm.assume(quantity > 0 && quantity < 1_000_000);
        vm.assume(salePrice > 0 && salePrice < 10 ether);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalValue}(fixedPriceMinter, tokenId, quantity, abi.encode(recipient), address(0), origin);
    ...
    function test_PaidMintRewardsWithFinderAndLister(uint256 quantity, uint256 salePrice) public {
        vm.assume(quantity > 0 && quantity < 1_000_000);
        vm.assume(salePrice > 0 && salePrice < 10 ether);
        ...
        vm.prank(buyer);
        target.mintWithRewards{value: totalValue}(fixedPriceMinter, tokenId, quantity, abi.encode(recipient), finder, origin);
    ...
    function testRevert_InsufficientEthForPaidMintRewards(uint256 quantity, uint256 salePrice) public {
        ...
        vm.prank(buyer);
        vm.expectRevert(abi.encodeWithSignature("INSUFFICIENT_ETH_FOR_REWARDS()"));
        target.mintWithRewards(fixedPriceMinter, tokenId, quantity, abi.encode(recipient), address(0), address(0));
    }
    ...
    function testRevert_WrongValueForSale(uint256 quantity, uint256 salePrice) public {
        ...
        (uint256 totalReward, , , ) = target.computePaidMintRewards(quantity);

        vm.expectRevert(abi.encodeWithSignature("WrongValueSent()"));
        target.mintWithRewards{value: totalReward}(fixedPriceMinter, tokenId, quantity, abi.encode(recipient), address(0), address(0));
    }
