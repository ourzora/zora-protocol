...
import { DeployCurrency, InitialPurchaseCurrency } from "@zoralabs/coins-sdk";
...
type CreateCoinArgs = {
  ...
  initialPurchase?: {       // Optional initial supply purchase
    currency: InitialPurchaseCurrency; // The currency to use for the initial purchase (InitialPurchaseCurrency.ETH)
    amount: bigint; // Amount of the coin to pre-purchase in InitialPurchaseCurrency (only ETH -> ZORA is supported currently)
    amountOutMinimum?: bigint; // Optional minimum amount of tokens to receive (slippage protection). Not typically needed for initial purchases of ETH or USDC.
  };
}
```
...
const coinParams = {
  ...
  initialPurchase: { // Optional: Purchase initial supply during creation
    ...
    amountOutMinimum: 0n, // Optional: minimum amount to receive (slippage protection)
  },
};
...
import { createCoinCall, DeployCurrency, InitialPurchaseCurrency } from "@zoralabs/coins-sdk";
import { Address } from "viem";
