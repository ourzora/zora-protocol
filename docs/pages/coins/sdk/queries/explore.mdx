...
async function fetchAllTopGainers() {
  ...
  try {
    ...
    do {
      ...
      if (response.data?.zora20Tokens && response.data?.zora20Tokens.length > 0) {
        allCoins = [...allCoins, ...response.data!.zora20Tokens];
      }
      ...
      if (!cursor || response.data?.zora20Tokens?.length === 0) {
        break;
      }
