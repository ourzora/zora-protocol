...
async function fetchUserProfile() {
  ...
  if (profile) {
    ...
    if (
      (profile?.linkedWallets && profile?.linkedWallets?.edges?.length) ||
      0 > 0
    ) {
...
type ProfileData = {
  profile?: {
    ...
    platformBlocked?: boolean; // Whether the profile is blocked on the platform (most applications should exclude blocked profiles from visibility)
    ...
    creatorCoin?: {
      // User's Creator Coin (if they have one)
    ...
    createdCoins?: {
      ...
      edges?: Array<{
        node?: {
          ...
          platformBlocked?: boolean; // Whether the coin is blocked on the platform (most applications should exclude blocked coins from visibility)
...
type Response = {
  balances?: Array<{
    ...
    token?: {
      ...
      uniqueHolders?: number; // Number of unique holders
      media?: {
        // Media associated with the coin
        previewImage?: string;
        medium?: string;
        blurhash?: string;
      };
