...
New coins have a **time-based dynamic fee** that protects against sniping bots immediately after creation. The fee decays linearly from **99% to 1%** over **10 seconds**.
...
- **Initial supply bypass**: When a coin is deployed with an initial supply purchase, that purchase uses the normal 1% fee (not the launch fee)
...
- **Purpose**: The launch fee discourages bots from front-running coin launches by making immediate purchases prohibitively expensive
- **After 10 seconds**: The fee settles to the standard 1% LP fee for all subsequent trades

<Callout type="info">
  The launch fee currently only applies to buys and sells in the first 10
  seconds after coin creation. After this period, the normal 1% fee structure
