...
This function creates a new coin contract with the specified parameters and its associated Uniswap pool (V3 or V4 depending on pool configuration).

The factory automatically determines the coin version and pool type based on the `poolConfig` parameter.
...
- `uri`: The coin metadata URI (should be an IPFS URI). See [Metadata](/coins/contracts/metadata) for more information.
...
- `message`: Optional message associated with the coin creation
...
- __Note:__ When creating a coin, the return values are not accessible and the [`CoinCreated`](#coin-created) or [`CoinCreatedV4`](#coin-created-v4) event needs to be parsed from the logs to find the resulting address.
